...
async def add_keyword_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    await update.message.reply_text("🔑 Отправь ключевое слово (например, deep house):")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user_id):
        if awaiting == 'channel':
            ...
            set_channel(ch)
            await update.message.reply_text(f"✅ Канал {ch} сохранён!", reply_markup=kb)
